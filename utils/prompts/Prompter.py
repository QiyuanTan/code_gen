...
class Prompter:
    def __init__(self):
        ...
        with open('prompts/planning_prompt.txt', 'r', encoding='utf-8') as file:
            # 2. 读取文件内容
            self.PLANNING_PROMPT = file.read()

        with open('team_description.txt', 'r', encoding='utf-8') as file:
            # 2. 读取文件内容
            self.TEAM_DESCRIPTION = file.read()

        with open('../coder.txt', 'r', encoding='utf-8') as file:
            # 2. 读取文件内容
            self.CODER = file.read()

        with open('../analyst.txt', 'r', encoding='utf-8') as file:
            # 2. 读取文件内容
